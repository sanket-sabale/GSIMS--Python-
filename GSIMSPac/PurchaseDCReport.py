...
class PurchaseDCReport:
    ...
    def __init__(self,canvas_widget):
        ...
        self.productNoLabel.configure(text="DC No:")
        ...
        self.reportTree.heading("DC No", text="DC No", anchor=W)
        self.reportTree.heading("DC Date", text="DC Date", anchor=W)
    ...
    def addTreeDataOfExp(self,event = "1"): 
        ...
        self.cursor.execute("select purchasedcid,purchasedcdate,dcno,dcdate,supplierid,suppliername,supplierlocation,qtytotal,total,status from purchase where status = %s",["dc"])
...
class PurchaseDailyReport:
    def __init__(self,canvas_widget):
        ...
        self.EntryNoLabel.configure(text="No:")
        ...
        self.Type.place(relx=0.53, rely=0.135) 
        self.Type['values'] = ('All','dc','inv')
        self.Type.current(0)
        self.Type.configure(state='readonly')
        self.Type.configure(width=20)
        self.Type.configure(font=f2)
        self.Type.bind("<<ComboboxSelected>>",self.addTreeDataOfSale)
        ...
        self.reportTree.heading("Product NO", text="Product NO", anchor=W)
        ...
        self.reportTree.heading("Available Qty", text="Available Qty", anchor=W)
        self.reportTree.heading("Amount", text="Amount", anchor=W)
        self.reportTree.heading("Status", text="Status", anchor=W)
        ...
        for productId in productIds:
            ...
            if self.billType.get() == "All":
                ...
                if self.Type.get() == "All":    
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and (status = %s or status = %s) and (purchasedcdate = %s or purchasedate = %s)",[productId[0],"dc","inv",date,date])
                else:
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and status = %s and (purchasedcdate = %s or purchasedate = %s)",[productId[0],typeof,date,date])
            else:
                typeof = self.Type.get()
                if self.Type.get() == "All":    
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and (status = %s or status = %s) and (purchasedcdate = %s or purchasedate = %s) and paytype = %s",[productId[0],"dc","inv",date,date,billType])
                else:
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and status = %s and (purchasedcdate = %s or purchasedate = %s) and paytype = %s",[productId[0],typeof,date,date,billType])
            ...
            amount = 0
            for item in data:
                purchaseQty = float(purchaseQty) + float(item[0])
                returnQty = float(returnQty) + float(item[1])
                availableQty = float(availableQty) + float(item[2])
                amount = float(amount) + float(item[3])

            listItem = [srno,productId[0],productId[1],purchaseQty,returnQty,availableQty,amount,status] 
            self.reportTree.insert("", "end", values=listItem,tags=("item",))

class PurchaseMonthlyReport:
    def __init__(self,canvas_widget):
    ...
    def addTreeDataOfSale(self,event = '1'):
        ...
        for productId in productIds:
            ...
            if self.billType.get() == "All":
                ...
                # here is some problem with product shampoo there is only one product is shown in table
                
                
                
                if self.Type.get() == "All":    
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and (status = %s or status = %s) and ((purchasedcdate >= %s and purchasedcdate <= %s) or (purchasedate >= %s and purchasedate <= %s))",[productId[0],"dc","inv",fromdate,todate,fromdate,todate])
                    # print("its work")
                else:
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and status = %s and ((purchasedcdate > %s and purchasedcdate < %s) or (purchasedate > %s and purchasedate < %s))",[productId[0],typeof,fromdate,todate,fromdate,todate])
                    # print("its work")
            else:
                # print("its work")
                typeof = self.Type.get()
                if self.Type.get() == "All":    
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and (status = %s or status = %s) and ((purchasedcdate > %s and purchasedcdate < %s) or (purchasedate > %s and purchasedate < %s)) and paytype = %s",[productId[0],"dc","inv",fromdate,todate,fromdate,todate,billType])
                else:
                    self.cursor.execute("select quantity,returnqty,availableqty,amount,status,productname from purchase where productno = %s and status = %s and ((purchasedcdate > %s and purchasedcdate < %s) or (purchasedate > %s and purchasedate < %s)) and paytype = %s",[productId[0],typeof,fromdate,todate,fromdate,todate,billType])
